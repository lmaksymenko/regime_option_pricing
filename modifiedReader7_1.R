...
### check script that doesnt use ks
### graph
#### red = trailing 30
#### blue = multi year
#### green = black scholes
...
fileRead <- function(ticker,folder) {
  ...
  openHours = strftime(seq.POSIXt(as.POSIXct("2015-08-01 09:30:00"), as.POSIXct("2015-08-01 16:00:00"),by = "1 min"), format = "%H:%M")
  cnt = 0
...
regimeRead <- function(df) {
  ...
  for (k in 2:78) {
    ...
    if (ks.test(set1$logret,set2$logret)$p.value < 0.05) {
      #New Regime
      ...
      #Same Regime
      carryOver = rbind(carryOver, set1)
    }
    if (k == 78) {
...
folders = c(list.dirs(".", FALSE, FALSE)[1:6], list.dirs(".", FALSE, FALSE)[8:13])
...
for (i in 1:length(folders)) {
  ...
  for (j in 1:length(stonks)){
    ...
    if (class(info) == "data.frame") {
      # Only previous years
      if (substr(folder, nchar(folder) - 3, nchar(folder)) != "2020") {
        ...
        write.csv(trail, paste(getwd(), "/trailing/", stonk, ".csv", sep = ''))
